...
The Jacobian $\fnof{J}{\xione,\xitwo}$ is given by \lnorm{}{\vectr{t}_{1} \times
\vectr{t}_{2}} (where $\vectr{t}_{1}$ and $\vectr{t}_{2}$ are the two tangent
...
If we recall what the elements of $\vectr{f}$ in \eqnref{eqn:FEM matrices}
contained, then we can convert $\vectr{q}$ in \eqnref{eqn:appliedmethod} to an
...
\bref{eqn:FEM matrices} yields $\matr{K}\vectr{u}=\matr{M}\vectr{q}$ an
