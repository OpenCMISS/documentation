...
\newcommand{\directionalderiv}[2]{\ensuremath{\del_{#1} #2}} % directional
                                % derivative
...
\newcommand{\linearisation}[2]{ %
  \ifthenelse{\equal{#1}{}}{ %
    \ensuremath{\fnof{L}{#2}}
  }{
    \ensuremath{\fnof{L_{#1}}{#2}} % 
  }} % linearisation e.g. \linearisation{}{u} => L(u) & \linearisation{x}{u} => L_x(u)
...
% Tensor operators
\DeclareMathOperator{\sympart}{sym} % symmetric part e.g., sym A
\DeclareMathOperator{\skewpart}{skew} % skew-symmetric part e.g., skew A
\DeclareMathOperator{\devpart}{dev} % deviatoric part e.g., dev A
\DeclareMathOperator{\sphpart}{sph} % spherical part e.g., sph A
...
\newcommand{\pushforward}[2]{\ensuremath{#1_{*}#2}} % push forward
                                % e.g. \pushforward{M}{v} => M_* v
\newcommand{\pullback}[2]{\ensuremath{#1^{*}#2}} % pull back
                                % e.g. \pullback{M}{v} => M^* v
...
\newcommand{\sharptensor}[1]{\ensuremath{#1^{\sharp}}} % sharp tensor e.g.,
                                % \sharptensor{A} => A^#
