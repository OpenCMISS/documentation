(function(){
	...
	render: function(){
		...
		<li><p>For the installer, run the exe; an install wizard will pop up, follow the steps and select ‘Add OpenCMISS-Zinc to the system PATH for all users’ (or add it manually for a single user as described below). For the zip archive, extract the contents of the archive where you want Zinc installed, and add its top folder to the PATH environment variable for the current user (In Windows 7 this is under Control Panel|User Accounts|Change my environment variables).</p></li>
